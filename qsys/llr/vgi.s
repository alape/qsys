...
.text_llr
    ...
    vgi_blit:
        ...
        _ycalcblit:
            add r5, r5, VGI_WIDTH       ; calculate Y offset: addr = addr + VGI_WIDTH * X
            add r6, r6, 1
            blt r0, r6, r1
    ...
    vgi_blit1bpp:
        ; Place (blit) a 1bpp (32 pixels per word) bitmap by specified coordinates in the VGI framebuffer:
        ;   R0: X coordinate,
        ;   R1: Y coordinate,
        ;   R2: Bitmap width,
        ;   R3: Bitmap height,
        ...
        ld r6, 0                        ; Y counter
        ld r7, $_xlineblit1bpp          ; util function vectors
        ld r0, $_ycalcblit1bpp
        ...
        _xlineblit1bpp:                 ; output a single line of bitmap to VGI framebuffer
            ld r0, r4                   ;   (copy words from bitmap to framebuffer until X counter reaches
            st r0, r5                   ;    the width of bitmap)
            add r6, r6, 1
            add r5, r5, 1
